...
pentaho.cda.Descriptor.prototype = {
	...
	,discoverQueries: function(func) {
		...
		if (that.queries.length == 0) {
			$.get("/pentaho/content/cda/listQueries", {path:that.path},
				function(data){
					for (var i=0,j=data.resultset.length;i<j;i++){
						var rs = data.resultset[i];
						//console.log(rs);
						that.queries[i] = new pentaho.cda.Query({id:rs[0], name:rs[1], type:rs[2]}, that);
					};
					func(that.queries);
				});
		} else {
...
pentaho.cda.Query.prototype = {
	discoverParameters: function(func){
		...
		if (this.parameters.length == 0) {
			$.get("/pentaho/content/cda/listParameters", {path:that.file.path, dataAccessId:this.id},
			function(data){
				for (var i=0,j=data.resultset.length;i<j;i++){
					var rs = data.resultset[i];
