		...
		this.sandbox.listen('prb-datasource-value-change',
			function(event) {
			     for (var i=0,j=event.data.length;i<j;i++){
			         qryarray[i] = [event.data[i].id, event.data[i].name];
			     }
...
pentaho.prb.DataSourceValueSelect.prototype.display = function(data) {
	...
	dsvalselect = {
		...
		parameter: "datasourceVal",
		  ...
		  htmlObject: that.element,
		  postChange:function(){
			that.change({name:datasourceVal, path:$('#' + that.element + ' :selected').text()});
			}	
		}
	...
	//console.log(data);
		switch (datasourceType) {
            ...
            case "mdx":
                break;
			case "cda":
				var cdaDesc = new pentaho.cda.Descriptor({name:data.name,path:data.path});
				cdaDesc.discoverQueries(function(queries){
				    qryarray = queries;
...
pentaho.prb.StateManager.prototype.loadState = function(data){
	...
	//console.log(data);
	var action = new pentaho.solRepo.Action({solution:data.solution, path:data.path, id:data.file})
	action.load(function(file) {
